...
class SendGoalAheadNode(Node):
    ...
    def execute(self):
        ...
        # ---- Step 3.5: 发布禁扫区，覆盖搜索区域使候选点被过滤 ----
        self.publish_special_terrain(robot_x, robot_y, yaw)
    ...
    # =========================================================
    def publish_special_terrain(self, robot_x, robot_y, yaw):
        """直接用绝对坐标覆盖搜索区域（后方）"""
        polygon = GeoPolygon()
        # 覆盖 find_vertices 区域: x∈[-4, -2.5], y∈[3.0, 4.0]
        coords = [
            (-4.0, 3.0),
            (-2.5, 3.0),
            (-2.5, 4.0),
        ...
        msg = Polygons()
        msg.polygons = [polygon]
        self.special_terrain_pub.publish(msg)

        self.get_logger().info('已发布禁扫区多边形(绝对坐标，覆盖搜索区域):')
        for i, pt in enumerate(polygon.points):
            self.get_logger().info(f'  禁扫区 P{i+1}: ({pt.x:.3f}, {pt.y:.3f})')
